...
const Sidebar = () => {
  ...
  const routers: IRoute[] = [
    ...
    },
    {
      href: '/hotels',
      label: 'Hotel',
      active: pathname.match('/hotels'),
      Icon: HotelIcon,
    },
    {
      href: '/amenities',
      label: 'Amenities',
      active: pathname.match('/amenities*'),
      Icon: MonitorIcon,
    },
  ]
            ...
            <div key={route.href} className='w-full'>
              <Link
                ...
                className={cn(
                  ...
                  route.active
                    ? 'bg-blue-600/50 text-white hover:bg-blue-600/30'
                    : 'text-zinc-400'
                )}
              >
                <div className='flex items-center justify-center w-12 h-12'>
                  <route.Icon
                    className={cn('w-4 h-4', route.active && 'text-blue-600')}
                  />
                </div>
                {open && (
                    ...
                    <div key={sr.href} className='ml-4'>
                      <Link
                        ...
                        className={cn(
                          'flex items-center text-sm font-medium hover:bg-zinc-800 w-full rounded-sm transition duration-200 my-2 active:scale-95',
                          sr.active
                            ? 'text-blue-600 hover:bg-blue-600/30'
                            : 'text-zinc-400'
                        )}
                      >
                        <div className='flex items-center justify-center w-12 h-12'>
                          <sr.Icon
                            className={cn(
                              'w-4 h-4',
                              sr.active && 'text-blue-600'
                            )}
                          />
                        </div>
                        {open && (
                          <div className='block ml-2 mr-2'>{sr.label}</div>
                        )}
                      </Link>
                    </div>
                  )
